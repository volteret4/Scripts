...
def search_files(query):
    ...
    for filename in glob.iglob(os.path.join(SEARCH_DIR, '**', '*.md'), recursive=True):
        ...
        # Clasificación según ruta (sin cambios)
...
def update_results(event):
    ...
    for filename, title in content_results:
        result_list.insert(tk.END, f"C {title}")  # Mostrar contenido como un solo título
...
def open_file_with_shortcut(event=None):
    """Abre el archivo seleccionado con VSCodium al presionar Ctrl+O."""
    open_file()  # Llama a la función que ya tienes para abrir el archivo
...
def create_gui():
    ...
    open_button = tk.Button(search_frame, text="Editar en VSCodium", command=open_file, bg='#f8bd9a')
    open_button.pack(side=tk.LEFT, padx=(0, 5))  # Mover a la izquierda

    delete_button = tk.Button(search_frame, text="Eliminar", command=delete_file, bg='#ff8b8b')
    delete_button.pack(side=tk.LEFT, padx=(0, 5))  # Mover a la izquierda
    ...
    root.bind("<Control-o>", open_file_with_shortcut)  # Para abrir archivo con Ctrl+O
