...
class MusicExplorer:
    def __init__(self, config_file='config.ini'):
        ...
        log_file = self.config.get('logging', 'file', fallback='/tmp/music_web.log')
    ...
    def get_artist_details(self, artist_id):
        """Obtiene detalles completos de un artista - VERSIÓN CORREGIDA"""
        conn = self.get_db_connection()
        if not conn:
            return None
        ...
        try:
            ...
            for album_row in cursor.fetchall():
                ...
                # Prioridad: album_art_path del álbum > album_art_path_denorm de canciones
    ...
    def get_folder_structure(self, path=""):
        """Obtiene la estructura de carpetas de música"""
        try:
            full_path = Path(self.music_root) / path if path else Path(self.music_root)
            ...
            for item in sorted(full_path.iterdir()):
                if item.is_dir():
                    relative_path = str(item.relative_to(self.music_root))
                    items.append({
                        'name': item.name,
                        'type': 'folder',
                        'path': relative_path,
...
# CORREGIR EN app.py - El problema está en los imports

# CAMBIAR ESTOS IMPORTS AL INICIO DE app.py:
# from datetime import datetime  # ❌ QUITAR SI ESTÁ
import time as time_module  # ✅ CAMBIAR ESTO
from datetime import datetime
import subprocess
import threading
from pathlib import Path

# Y EN EL ENDPOINT download_album, CAMBIAR:

@app.route('/api/album/<int:album_id>/download', methods=['POST'])
def download_album(album_id):
    """Endpoint para descargar álbum al servidor web - VERSIÓN CORREGIDA"""
    ...
    try:
        ...
        # CORREGIR AQUÍ: usar time_module en lugar de time
        ...
        def perform_download():
            try:
                ...
                rsync_cmd = [
                    ...
                    '-avzh',
                    '--progress',
                    f"pepe@pepe-cono:{album_path}/",
                    '/mnt/dietpi_userdata/suarez_mula/musica/'
                ]
                
                explorer.logger.info(f"Ejecutando descarga: {' '.join(rsync_cmd)}")
                ...
                while True:
                    ...
                    if output:
                        # Actualizar status con el progreso
                ...
                if return_code == 0:
                    download_status[download_id] = {
                        'status': 'completed',
                        'progress': 100,
                        'message': f'Álbum "{album["name"]}" descargado exitosamente a /mnt/dietpi_userdata/suarez_mula/musica/',
                        'album_name': album['name'],
                        'artist_name': album['artist_name']
                    }
        ...
        return jsonify({
            ...
            'album_name': album['name'],
            'artist_name': album['artist_name'],
            'album_path': str(album_path)  # Para debug
        })
...
if __name__ == '__main__':
    ...
    host = explorer.config.get('web', 'host', fallback='127.0.0.1')
