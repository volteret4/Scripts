...
def buscar_en_jackett(artista, album, jackett_url, jackett_api_key, lidarr_url, lidarr_api_key, indexador="rutracker", solo_flac=False):
    """
    Busca torrents directamente en Jackett para el artista y álbum, con opción de filtrar por FLAC.
    Utiliza MusicBrainz para obtener nombres estandarizados de álbumes cuando es posible.
    """
    print(f"Buscando torrents para '{artista} - {album}' en Jackett ({indexador}){' (solo FLAC)' if solo_flac else ''}...")
    
    # Intentar obtener información estandarizada de MusicBrainz
    artista_info, album_info = obtener_info_musicbrainz(artista, album, lidarr_url, lidarr_api_key)
    
    # Si tenemos información estandarizada, usarla para la búsqueda
    if album_info and 'standardized_title' in album_info:
        busqueda_artista = artista_info.get('artistName', artista)
        busqueda_album = album_info.get('standardized_title')
        print(f"Usando información estandarizada para la búsqueda: '{busqueda_artista} - {busqueda_album}'")
    else:
        busqueda_artista = artista
        busqueda_album = album
        
        # Si no tenemos información de MusicBrainz, intentar normalizar manualmente el nombre del álbum
        # Esto podría ser útil como fallback
        import re
        busqueda_album = re.sub(r'\s*\([^)]*(?:Edition|Deluxe|Remaster|Anniversary|Special|Bonus|Version|Expanded)[^)]*\)', '', busqueda_album, flags=re.IGNORECASE)
        busqueda_album = busqueda_album.strip()
        
        print(f"Usando información normalizada manualmente: '{busqueda_artista} - {busqueda_album}'")
    
    url = f"{jackett_url}/api/v2.0/indexers/{indexador}/results/torznab"
    
    # Si queremos solo FLAC, lo añadimos a la búsqueda
    query = f"{busqueda_artista} {busqueda_album}"
    if solo_flac:
        query += " flac"
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            # Verificar coincidencia con el nombre estandarizado del álbum
            coincidencia_exacta = False
            
            # Comprobar si el título coincide exactamente con el patrón "artista - album"
            if f"{busqueda_artista.lower()} - {busqueda_album.lower()}" in titulo.lower():
                coincidencia_exacta = True
            
            # También considerar coincidencia si contiene el nombre estandarizado del álbum
            elif busqueda_album.lower() in titulo.lower() and busqueda_artista.lower() in titulo.lower():
                coincidencia_exacta = True
    ...
    for i, item in enumerate(playlist_data, 1):
        ...
        if album_key in album_info_cache:
            ...
            resultados = buscar_en_jackett(busqueda_artista, busqueda_album, 
                                        ...
                                        lidarr_url, lidarr_api_key,  # Nuevos parámetros
                                        indexador, solo_flac)
...
def iniciar_script_fondo(num_torrents, num_canciones, output_path, json_file, temp_server_port=8584):
    """Inicia el script y muestra su salida en tiempo real, pasando el número total de canciones."""
    try:
        # Verificar que los argumentos no son None
        if not all([output_path, json_file]):
            missing = []
            if not output_path:
                missing.append("output_path")
            if not json_file:
                missing.append("json_file")
            print(f"\nError: Los siguientes parámetros requeridos son nulos: {', '.join(missing)}")
            return None
            
        # Primero buscar el script en la ubicación relativa
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '3_servidor_playlist.py')
        
        # Si no existe, buscar en una ruta absoluta alternativa
        if not os.path.exists(script_path):
            alternative_path = "/storage/playlist_popollo/3_servidor_playlist.py"
            if os.path.exists(alternative_path):
                script_path = alternative_path
                print(f"Usando ruta alternativa para el script: {script_path}")
            else:
        ...
        # Asegurar que todos los valores son strings donde corresponde
        num_torrents_str = str(num_torrents) if num_torrents is not None else "0"
        num_canciones_str = str(num_canciones) if num_canciones is not None else "0"
        temp_server_port_str = str(temp_server_port) if temp_server_port is not None else "8584"
        
        # Ejecutar el script directamente (no en segundo plano)
        cmd = [
            'python', 
            script_path,
            "--numero-torrents", num_torrents_str,
            "--numero-canciones", num_canciones_str,
            "--json-file", json_file,
            "--output-path", output_path,
            "--temp_server_port", temp_server_port_str
        ]
        
        print(f"\nEjecutando script con comando: {' '.join(cmd)}")
        
        # Usar run en lugar de Popen para ver la salida en tiempo real
        # Sin check=True para evitar que una excepción detenga el programa principal
...
def obtener_info_musicbrainz(artista, album, lidarr_url, lidarr_api_key):
    """
    Obtiene información de MusicBrainz a través de Lidarr, incluyendo el release group
    para obtener un nombre estandarizado del álbum.
    """
    print(f"\nBuscando información de MusicBrainz para '{artista} - {album}'...")
    
    try:
        # Primero obtenemos el MBID del artista y álbum a través de Lidarr
        artista_info, album_info = buscar_en_lidarr(artista, album, lidarr_url, lidarr_api_key)
        
        if not artista_info or not album_info:
            print("No se pudo obtener información de Lidarr.")
            return None, None
        
        # Intentar obtener el release group ID del álbum
        release_id = album_info.get('foreignAlbumId')
        
        if not release_id:
            print("No se encontró el ID de release en Lidarr.")
            return artista_info, album_info
        
        # Consultar la API de MusicBrainz para obtener el release group
        mb_url = f"https://musicbrainz.org/ws/2/release/{release_id}?inc=release-groups&fmt=json"
        
        headers = {
            'User-Agent': 'AlbumSearchScript/1.0 (+https://github.com/volteret4/Scripts)'
        }
        ...
        # Extraer la información del release group
        release_group = mb_data.get('release-group', {})
        release_group_title = release_group.get('title')
        release_group_id = release_group.get('id')
        
        if release_group_title:
            print(f"Nombre estandarizado del álbum: '{release_group_title}'")
            # Actualizar el título del álbum en album_info
            album_info['standardized_title'] = release_group_title
            album_info['release_group_id'] = release_group_id
        ...
        print(f"Error obteniendo información de MusicBrainz: {e}")
        return None, None
...
def clear_screen():
    """Limpia la pantalla de la terminal en cualquier sistema operativo"""
    import os
    # Para Windows
    if os.name == 'nt':
        os.system('cls')
...
def main():
    ...
    parser.add_argument('--numero_torrents', help='Numero de torrents descargados para pasar al tercer script')
    ...
    if hasattr(args, 'numero_torrents') is not None:
        config["numero_torrents"] = args.numero_torrents
    ...
    if config.get("json_file"):
        ...
        resultados = buscar_en_jackett(busqueda_artista, busqueda_album, 
                                    ...
                                    config["lidarr_url"], config["lidarr_api_key"],  # Nuevos parámetros
    ...
    if torrents_descargados and config.get("output_path"):
        ...
        try:
            ...
            if isinstance(temp_server_port, str) and temp_server_port.isdigit():
                temp_server_port = int(temp_server_port)
            elif not isinstance(temp_server_port, int):
                temp_server_port = 8584  # Valor por defecto
                
            proceso_fondo = iniciar_script_fondo(
                len(torrents_descargados),
                num_canciones,
            ...
            if proceso_fondo is None:
                print("No se pudo iniciar el script 3_servidor_playlist.py")
            else:
                print(f"\nProceso del script iniciado con código de retorno: {proceso_fondo.returncode}")
        except Exception as e:
            print(f"\nError al iniciar el script: {e}")
            import traceback
            traceback.print_exc()
