...
TODOTXT_CFG_FILE="${HOME}/.config/TODO_albums/todo.cfg"

CONFIG_FILE="${HOME}/.config/TODO_albums/todofish.conf"
...
openurl() {
    # Obtener el número de línea de la tarea seleccionada
    lineno=$(getlinenumber "$1")
    
    # Obtener el contenido de la tarea
    task_content=$(runtodo ls | grep -P "^$lineno ")

    # Extraer la URL de la tarea (puedes ajustar esto según el formato de tus tareas)
    url=$(echo "$task_content" | grep -oP '(http[s]?://\S+)')
    
    # Abrir la URL en el navegador predeterminado (puedes ajustar esto según tu sistema)
    xdg-open "$url"  # Linux
    # open "$url"  # macOS
    # start "$url"  # Windows
}
...
option() {
    ...
    do
        if [[ ${current_line:0:1} == 'x' ]]; then
            selection=$(echo -e "1. Open URL|2. Not implemented" | runrofi -sep "|" -kb-accept-entry "Return" -mesg "Item: ${current_line}" -dmenu -p "Action")
        else
            selection=$(echo -e "1. Mark Done|2. Edit|3. Edit priority|4. Remove priority|5. Delete|6. Open URL" | runrofi -lines 6 -sep "|" -u 4 -a 0 -kb-accept-entry "Return" -mesg "Item: ${current_line}" -dmenu -p "Action")
        fi
